...
function createControls() {
  ...
  const N = createRange({
    min: 2, max: 100, value: 20, step: 1, label: "Periods (N)",
    onChange: () => redraw()
  });

  const K = createRange({
    min: 0, max: 10, value: 2, step: 0.1, label: "Deviations (K)",
    onChange: () => redraw()
  });

  container.appendChild(N.element);
  container.appendChild(K.element);
  container.appendChild(document.createElement("br"));
  document.getElementById("chart").before(container);

  return { getN: N.getValue, getK: K.getValue };
}

function createRange({ min, max, value, step, label, onChange }) {
...
function redraw() {
  ...
  chartDiv.innerHTML = "";
  chartDiv.appendChild(drawChart(_data, controls.getN(), controls.getK()));
}
